...
void mp4diag_video_metadata(const VideoMetadata* meta)
{
    ...
    if (!meta->sps || meta->sps_len <= 0) {
        ...
        if (meta->sps_len >= 4) {
            ...
            if (nal_type == 7) {
                ...
                // Profile
                uint8_t profile_idc = meta->sps[1];
                ...
                // Constraint flags
                uint8_t constraints = meta->sps[2];
    ...
    if (meta->sps && meta->sps_len >= 2) {
        uint8_t profile = meta->sps[1];
        printf("  ");
