...
// --- SPS + PPS SEM start code (NAL puro) --- Para testes
const uint8_t SPS_PPS_RAW[] = 
{
    // SPS (NAL type 7)
    0x00, 0x00, 0x00, 0x01,
...
static MediaBuffer* _build_avcC(const VideoInitData* v)
{
...
static MediaBuffer* _build_hvcC(const VideoInitData* v)
{
...
static MediaBuffer* _build_stsd_box(const VideoInitData* v)
{
    ...
    if (v->hdot_version == HDOT_H264)
    {
        ...
        // wrap avcC as box
        mbuffer_append_uint32(sample_entry, (uint32_t)(8 + avcc->Size));
        ...
        if (avcc->Size)
        {
            mbuffer_append(sample_entry, avcc->Data, avcc->Size);
        }
    ...
    else if (v->hdot_version == HDOT_H265)
    {
        ...
        mbuffer_append_uint32(sample_entry, (uint32_t)(8 + hvcc->Size));
        mbuffer_append_string(sample_entry, "hvcC");

        if (hvcc->Size)
        {
            mbuffer_append(sample_entry, hvcc->Data, hvcc->Size);
        }
    ...
    mbuffer_append_uint32(sample_entry_box, (uint32_t)(8 + sample_entry->Size));
    mbuffer_append_string(sample_entry_box, type);
    mbuffer_append(sample_entry_box, sample_entry->Data, sample_entry->Size);
    mbuffer_release(&sample_entry);

    // agora adiciona sample_entry_box aos stsd_payload
    mbuffer_append(pay, sample_entry_box->Data, sample_entry_box->Size);
...
static MediaBuffer* _build_moov_box(const VideoInitData* v) // uint32_t timescale
{
    ...
    mbuffer_append(dref, urlbox->Data, urlbox->Size);
...
MediaBuffer* _concod_read_init_segment(const VideoInitData* vid)
{
    MediaBuffer* moov = build_moov_box(vid);
    MediaBuffer* output = mbuffer_create(moov->Size + sizeof(FTYP_BOX));
    mbuffer_append(output, FTYP_BOX, sizeof(FTYP_BOX));
    mbuffer_append(output, moov->Data, moov->Size);
...
// saida como buffer...
MediaBuffer* _concod_read_frame(const FrameIndex* frame, FILE* src)
{
